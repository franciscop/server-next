...
- **`body`**: `'parse' | 'raw' | 'stream'`
  - How request bodies are read into `ctx.body`. Defaults to `parse`. See [`body`](#body).
...
- **Default**: `'parse'`
- **Description**: Controls how the request body is read into [`ctx.body`](5.%20Context.md#ctxbody). Set it globally, or per route where it merges like any other route option (local wins):
...
export default server({ uploads: './uploads' })
  ...
  .post('/webhook', { body: 'raw' }, (ctx) => {
    if (!verify(ctx.body, ctx.headers['x-signature'])) return status(401);
    return status(200);
  })
  // stream a large upload to a per-request folder, after auth, with no buffering
  .post('/videos/:id', { body: 'stream' }, requireUser, async (ctx) => {
    await ctx.options.uploads.folder(ctx.url.params.id).write('video.mp4', ctx.body);
    return status(201);
  });
```
