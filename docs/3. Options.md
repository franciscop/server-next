...
Security-related settings, grouped under one option. Server.js applies secure defaults on its own: a set of response headers on every response (routes, static files, 404s and errors alike), and a check that rejects route params trying to climb the path. Pass `security: false` to turn them all off, or an object to override or extend individual pieces. Cross-origin access is its own option, [`cors`](#optionscors), since it opens access up rather than locking it down.
