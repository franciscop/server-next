...
- **[`public`](#public)** (`string | Bucket`): directory or bucket of static assets.
- **[`uploads`](#uploads)** (`string | Bucket | UploadPipeline`): where uploaded files are stored.
...
- **[`cors`](#cors)** (`boolean | Origin | Cors`): Cross-Origin Resource Sharing settings.
- **[`auth`](#auth)** (`AuthString | Auth`): authentication method and provider, e.g. `cookie:github`.
- **[`cookies`](#cookies)** (`Store`): store for signed cookie persistence.
- **[`onError`](#onerror)** (`(error, ctx) => Response`): handler for uncaught middleware errors.
- **[`log`](#log)** (`'info' | boolean`): built-in startup, connection and request logs.
- **[`favicon`](#favicon)** (`string | Bucket`): icon served at `/favicon.ico`.
