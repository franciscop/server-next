...
The request body. By default it is parsed by content-type; the [`body` option](3.%20Options.md#body) controls how it is read.
...
- For file uploads, the field value is an `UploadedFile` object `{ name, id, path, type, size }` (or an array of them for a repeated field name). Files stream to `uploads` as they arrive, so their bytes are never all in memory. See [File handling](2.%20Guides.md#file-handling).
...
The other [`body` modes](3.%20Options.md#body) give `ctx.body` a different shape, the same on every runtime (Node and the web runtimes):
...
.get('/report', async (ctx) => {
  ...
  ctx.time('query');          // time from the start of the request to here
  const html = render(rows);
  ctx.time('render');         // time since the previous mark
  return html;
});
...
The active WebSocket connection, available inside [`.socket()`](2.%20Guides.md#websockets) handlers. Use `ctx.socket.send(data)` to send a message to this client. See the [WebSockets guide](2.%20Guides.md#websockets).

## ctx.sockets

Every open WebSocket connection, available inside [`.socket()`](2.%20Guides.md#websockets) handlers. Iterate it to broadcast: `for (const socket of ctx.sockets) socket.send(data)`.
