...
Returning a number is the shorthand for `status(n).send()`, so `() => 201` and `() => status(201).send()` are the same. Reach for `status()` when you also need a body or other helpers.
...
Use this when the value depends on the request; for a fixed default across routes, set the [`cache` option](#cache) instead. A `Cache-Control` set here always wins over that default. Note the response is cached only when it's a GET with a `200` status.
