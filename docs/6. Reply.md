...
- [`json`](#json): stringify the objects and send them along `Content-Type: application/json`.
- [`redirect`](#redirect): send a `302` (or custom) redirect to another URL through the `Location` header.
...
- [`file`](#file): stream a file from disk, guessing its `Content-Type` and returning `404` if missing.
