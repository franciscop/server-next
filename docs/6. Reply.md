...
<details title="Do I need status() for a bare code?">
No. Returning a number is the shorthand for `status(n).send()`, so `() => 201` and `() => status(201).send()` are the same. Reach for status() when you also need a body or other helpers.
</details>
