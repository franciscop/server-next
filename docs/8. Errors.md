...
A **4xx** describes what the client got wrong, so its message is sent to them. A **5xx** describes what went wrong inside the app, so the client only gets `Server Error` and the real message, its hint and a link to this reference go to your log. That holds for errors your own handlers throw too: a `throw new Error(...)` has no status, so it is a 500, so its message never leaves the server.

|  | client receives | log receives |
|---|---|---|
| `4xx` | the message | nothing, it isn't your bug |
| `5xx` | `Server Error` | message, hint, docs link |
| a handler's own `throw` | `Server Error` | its message |
...
An error's own `status` is used only when it is a real error status (400 to 599). Anything else is a `500`: a `200` would tell the client the request worked, and a code outside the HTTP range cannot be sent at all. A thrown `Response` is a `500` too, since a response is something to return, not to throw.

## In development

When `NODE_ENV` is not `production` and the request came from a browser, an error renders as a page with the message, the hint, the stack and a link to this reference. Anything else (a `fetch`, `curl`, an SDK) still gets the plain body, so your API behaves the same in every environment.
...
`400` Invalid parameter "{name}": expected {type}, got "{value}"

A [typed route parameter](/documentation/router#parameters) (`:id(number)`, `:day(date)`) could not be cast from the URL. The route matched on its shape, so this is a `400` on a real route rather than a `404`. Drop the type to accept the value as a string.
...
Set or lower `uploads: { bucket, minFileSize: '1kb' }`.

### UPLOAD_NOT_ALLOWED

`403` Not allowed to upload files here

`uploads: { bucket, validate }` refused this request. `validate` runs before the body is read, so nothing was stored. Throw your own error from it to say more, or return anything but `false` to allow the request.
