...
A signed-in browser is just a session record plus its cookie, so seed the [`sessions`](/documentation/options#optionssessions) and [`auth.users`](/documentation/options#optionsauth) stores and send the cookie (or the `Bearer` id, under the `token` strategy):
