...
```js
export default server()
  .use(logger)        // a middleware: runs for every request
...
const requireAdmin = (ctx) => {
  if (!ctx.url.pathname.startsWith('/admin/')) return;  // not our concern, skip
...
export default router()
  .get('/users', () => User.list())
  .get('/users/:id(number)', (ctx) => User.find(ctx.url.params.id))
  .post('/users', (ctx) => User.create(ctx.body))
  .put('/users/:id(number)', (ctx) => User.update(ctx.url.params.id, ctx.body))
  .delete('/users/:id(number)', (ctx) => User.delete(ctx.url.params.id));
```
