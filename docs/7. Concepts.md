...
The built-ins use the same mechanism, not a privileged one. `server()` registers its own middleware at construction (timing, CORS preflight, static assets, sessions, and auth's user loading), which is why they sit before your functions in every chain, and why `ctx.user` is already loaded by the time your first middleware runs. The one framework step that is *not* a middleware is [validation](/documentation/router#validation): a route's schemas run after the body is read and before its chain, so even your guards only ever see validated values.

The two deliberate departures from "one flat chain per route":

- **Unmatched requests** run the global `.use()` middleware on their own, which is how static files and `favicon` answer requests that aren't routes, and how a catch-all `.use()` can respond to anything.
...
- **Wiring, not requests.** [`auth`](/documentation/options#optionsauth), [`sessions`](/documentation/options#optionssessions), `port` and `log` describe how the app is assembled (providers, stores, the listener), not how one request behaves. Per-route *authorization* is a middleware concern, a guard in the chain, not an option.
