...
- **Wiring, not requests.** [`auth`](/documentation/options#optionsauth), `port` and `log` describe how the app is assembled (providers, callbacks, the listener), not how one request behaves. Per-route *authorization* is a middleware concern, a guard in the chain, not an option.
