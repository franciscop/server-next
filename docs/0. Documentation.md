...
`store`: almost anything you want to persist will need a KV store to do so. For dev a plain `Map` works, but for production you would normally use something like Redis. See [Stores](#stores).
...
Two libraries ship with Server.js, so `npm install @server/next` is all you need:
...
For the common cases you pass a plain value and Server.js builds the right thing:

```js
server({ store: new Map(), uploads: './uploads' });
```

For anything else, import them from `@server/next` and build it yourself:
...
const uploads = bucket.S3('my-bucket', { id: process.env.S3_ID, key: process.env.S3_KEY });

export default server({ store, uploads });
```

You can also pass your own adapter: anywhere a store is expected, any object with the Polystore interface works, and anywhere a bucket is expected, any object with the `Bucket` interface (`file(name)` returning a file handle, and `folder(prefix)`).
...
A store is a key-value interface used to persist sessions, auth tokens, and other data. In development a plain `Map` is enough:

```js
export default server({ store: new Map() })
  .get('/', () => 'Hello');
```

Stores are built with [Polystore](https://polystore.dev/), exported as `kv`. Use it when you need to configure the store yourself, for instance to scope keys with a prefix or to set a default expiry:
...
export default server({ store, uploads, auth: 'cookie:github' })
...
- **[`polystore`](#stores) and [`bucket`](#file-handling)** replace the built-in adapters, for `store`/`session` and `uploads`/`public`/`favicon` respectively. Both ship with Server.js, exported as `kv` and `bucket`.
...
|, | `store`, `uploads`, `cors`, `auth`, `openapi`, `onError`, `onResponse`, `cache` (new) |
