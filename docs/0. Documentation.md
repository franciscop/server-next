...
That's everything: key-value stores and file storage come with it, so logins work out of the box and `uploads` takes a folder path. See [Dependencies](#dependencies) for Redis, S3 and the rest.
...
[`auth`](/documentation/options#optionsauth): the `users` and `sessions` stores default to in-memory `Map`s. For production you would normally use something like Redis.
...
Server.js ships with its two storage libraries included, so there's nothing extra to install for the built-ins: [`polystore`](https://polystore.dev/) powers the key-value stores (`auth.users`, `auth.sessions`), and [`bucket`](https://bucketjs.com) the file storage (`uploads`, `public`). Both are re-exported:

```js
import server, { kv, bucket } from '@server/next';
```

The simple forms need neither import: the auth stores take a plain `Map` or a raw client, and `uploads` takes a folder path. Reach for the exports to compose them, like `kv(redis).prefix('session:')` or `bucket.S3(...)`, and install only the backend client you actually use (`redis`, an S3 key, ...).
...
- **[`polystore`](https://polystore.dev/) and [`bucket`](#file-handling)** replace the built-in adapters, for `auth.users`/`auth.sessions` and `uploads`/`public` respectively. Both ship with Server.js, exported as `kv` and `bucket`.
...
| `session` | *removed*: there is no per-device data bag. Logins live in [`auth.sessions`](#optionsauth); keep your own state in a cookie or your database |
