...
> - **Uploads must be configured**: 1.x wrote to a temp folder by default; 2.x refuses a request carrying a file until you set [`uploads`](#optionsuploads), or `uploads: false` to ignore file fields.
...
New in 2.x: [`ctx.user`](#ctxuser), [`ctx.platform`](#ctxplatform), [`ctx.signal`](#ctxsignal), [`ctx.socket` / `ctx.sockets`](#ctxsocket), `ctx.app`.
...
| `session` | *removed*: there is no per-device data bag. Logins are whatever [`onLogin`](#optionsauth) stores; keep your own state in a cookie or your database |
| `parse` | [`parser`](#optionsparser), `parse`/`raw`/`stream`; the size cap is [`security.maxBodySize`](#optionssecurity) (1mb default) |
