...
[`store`](/documentation/options#optionsstore): almost anything you want to persist will need a KV store to do so. For production you would normally use something like Redis.
...
- **[`polystore`](/documentation/options#optionsstore) and [`bucket`](#file-handling)** replace the built-in adapters, for `store`/`session` and `uploads`/`public`/`favicon` respectively. Both ship with Server.js, exported as `kv` and `bucket`.
