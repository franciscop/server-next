...
Server.js ships with its two storage libraries included, so there's nothing extra to install for the built-ins: [`polystore`](https://polystore.dev/) powers the key-value stores (`sessions`, `auth.users`), and [`bucket`](https://bucketjs.com) the file storage (`uploads`, `public`, `favicon`). Both are re-exported:
...
- **[`polystore`](https://polystore.dev/) and [`bucket`](#file-handling)** replace the built-in adapters, for [`sessions`](#optionssessions)/`auth.users` and `uploads`/`public`/`favicon` respectively. Both ship with Server.js, exported as `kv` and `bucket`.
