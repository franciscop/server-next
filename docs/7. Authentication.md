...
For a step-by-step setup, see the [Login with GitHub](#login-with-github) guide. This page documents:

- [Configuring auth](#configuring-auth) — the object form and its fields.
- [Users and sessions](#users-and-sessions) — the two stores auth derives.
- [OAuth providers](#oauth-providers) — providers and their env vars.
- [Email and password](#email-and-password) — the built-in `email` provider.
- [Strategies](#strategies) — `cookie`, `token`, `jwt`, `key`.
- [Shared-secret key](#shared-secret-key) — the provider-less `key` strategy.
- [The user object](#the-user-object) — `ctx.user` by strategy.
- [Protecting routes](#protecting-routes) — guarding with `ctx.user`.
- [Responses](#responses) — status codes.
- [Routes](#routes) — every registered route.
...
Set each provider's authorized callback URL to `https://<your-host>/auth/callback/<provider>`. Login begins at `GET /auth/login/<provider>`; the server handles the callback, upserts the user, and (for `cookie`) redirects to `redirect`. The flow is CSRF-protected with a short-lived `state` cookie.

**Apple** signs its client secret as a short-lived ES256 JWT from `APPLE_PRIVATE_KEY` (no extra dependencies), and its callback is a `POST`.
...
- **`jwt`** (stateless APIs): login responds with a `token` that is a signed JWT (HS256, using your [`secret`](#optionssecret)) carrying the session itself. No server-side session is stored, so it scales without a shared store; the trade-off is you cannot revoke a single token before it expires (one week), and changing `secret` invalidates all of them. Sent the same way, as `Authorization: Bearer <jwt>`.
- **`key`** (machine-to-machine): a single shared secret, no users or sessions — see [Shared-secret key](#shared-secret-key).

> **Note — `jwt` needs a stable `SECRET`.** JWTs are signed with your [`secret`](#optionssecret). With none set, a random one is generated per process, so every token breaks on restart and across instances (with a startup warning). Because there's no server-side record, a leaked JWT is valid until it expires and rotating `secret` is the only way to revoke all tokens at once — prefer `cookie`/`token` if you need instant, per-session revocation.
