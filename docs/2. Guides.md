...
The `auth` value is `'<strategy>:<provider>'`. The strategy is `cookie` (a browser session cookie) or `token` (a bearer token for APIs), see [Strategies](#strategies-cookie-vs-token). The provider is one of `email`, `github`, `google`, `microsoft`, `discord`, `facebook`, `apple`, and you can combine several with a pipe (`cookie:google|github`).
