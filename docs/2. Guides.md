...
Turn on auth with the [`auth` option](#auth), a `<strategy>:<provider>` string, plus a `store` to keep users in. This registers the provider's routes and loads the signed-in user onto [`ctx.user`](#ctxuser) for every request:
...
See the [`auth` option](#auth) for the full provider list and the environment variables each one needs.
