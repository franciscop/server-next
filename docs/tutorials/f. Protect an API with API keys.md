...
With [`parser: 'stream'`](/documentation/options#optionsparser), `ctx.body` is the unread request stream: nothing is processed until `requireKey` has passed, and the handler pipes it straight to the bucket without buffering.
...
Swap the `Map` for a [polystore](https://polystore.dev/) store to manage keys at runtime.
