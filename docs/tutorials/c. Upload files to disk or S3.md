...
A missing file returns `404` for you. See [Serving stored files](/documentation#serving-stored-files) for signed URLs and cloud buckets.

## Next steps

- Store files in an S3-compatible [bucket](/documentation#file-handling) for production.
- Accept several files by repeating the field name, `ctx.body.photos` becomes an array.
- Give each kind of file its own destination and limits with [per-route `uploads`](/documentation#per-route-destinations), like `.post('/video', { uploads: videos }, ...)`.
