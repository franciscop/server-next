...
Keep the notes in a `Map` for now (swap it for a store built with [`kv`](https://polystore.dev/) like Redis to persist them). Return a value from a handler and it is sent as JSON; return a number and it becomes the status code.
...
- Persist notes in a store built with [`kv`](https://polystore.dev/) (Redis, the filesystem, and more) instead of a `Map`.
