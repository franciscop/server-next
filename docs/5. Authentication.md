...
Signing in means going through a provider. Email and password is out of scope: storing and checking passwords well is a product of its own, so for that use [Better Auth](#a-library), which plugs straight in.
...
Refuse a login by **throwing a [`ServerError`](/documentation/errors#throwing-your-own)**. The visitor lands on [`redirect.error`](#redirect) with its code, `?error=ACCOUNT_SUSPENDED`, and your page turns that code into words. Only codes travel in the URL, never text, so a crafted link cannot put a message of someone else's on your page. Anything else that fails in the callback (a plain `Error`, the provider down, a bug) is logged for the operator and arrives as `?error=LOGIN_FAILED`. When the visitor declines at the provider, its OAuth error comes through uppercased: `?error=ACCESS_DENIED`.

Refusing is never "return nothing": a missing `return` is `undefined` in JavaScript, so treating that as a denial would turn an ordinary bug into people being rejected with no stated reason.
...
It is named for what it returns, which is always the user; what it receives is whatever `onLogin` identified. On a **later request**, returning `undefined` means "no such user any more" and signs that credential out: that is how a deleted row or session revokes a login. At **login time** an `undefined` return can only be a bug, since `onLogin` just stored that id. Under `cookie` and `jwt`, where the user is signed into the credential, a nullish return there fails the login rather than minting an empty one; under `session` and `token` the credential carries only the id, so it is resolved on the next request instead.
...
Runs on `POST /auth/logout`, with the same id `getUser` receives, whichever strategy you use. The credential is cleared either way; this is for anything of yours that should go with it. Under `token` the credential lives in the client rather than in a cookie, so clearing it is the client's job: this callback is where you record the revocation your `getUser` then honours. A `jwt` is checked by its signature alone, so nothing recorded here stops a copy from working until it expires.
...
Mounted only by a login flow. [Checking a token minted elsewhere](#checking-a-token-minted-elsewhere), a function and a library instance mount nothing of ours.
...
A fragment rather than a query string, because browsers never send it to a server, so it stays out of access logs and referrer headers. It does stay in the browser's history, though, so read it and clear it as soon as the page loads:
