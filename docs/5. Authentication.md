...
That is the floor, and it is genuinely deployable. There is a [step by step version](/tutorials/2-sign-in-with-github) of it, and a [tutorial per setup](#tutorials) further down.
...
|----------|-----------------|
| [Sign in with GitHub](/tutorials/2-sign-in-with-github) | The smallest login: a cookie, no database |
| [Github and Google in one app](/tutorials/20-github-and-google-in-one-app) | Two providers, one account, linked by email |
| [Google login persisted in SQLite](/tutorials/21-google-login-persisted-in-sqlite) | Your own users table, with a role column |
| [Github login persisted in Redis](/tutorials/22-github-login-persisted-in-redis) | The same two callbacks against a key-value store |
| [Revocable sessions in Postgres](/tutorials/23-revocable-sessions-in-postgres) | A row per login, so logout ends it everywhere |
| [Discord login with bearer tokens](/tutorials/24-discord-login-with-bearer-tokens) | The `jwt` strategy, for a SPA or a native app |
| [Supabase tokens with your own users](/tutorials/25-supabase-tokens-with-your-own-users) | Verifying a hosted token, mapped to your table |
| [Clerk tokens in a same-origin cookie](/tutorials/26-clerk-tokens-in-a-same-origin-cookie) | Reading a vendor's cookie, and the `azp` claim |
| [Firebase tokens from a mobile app](/tutorials/27-firebase-tokens-from-a-mobile-app) | Verifying Firebase and Identity Platform |
| [Keycloak SSO by issuer URL](/tutorials/28-keycloak-sso-by-issuer-url) | Any OIDC issuer, including self-hosted |
| [Github scopes and the access token](/tutorials/29-github-scopes-and-the-access-token) | Calling a provider's API on someone's behalf |
