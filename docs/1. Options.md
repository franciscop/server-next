...
Strategies are `cookie` (a session cookie), `token` (an opaque bearer token), and `jwt` (a self-contained, signed bearer token). For machine-to-machine access with a shared secret, use a plain middleware instead; see the [API keys](/tutorials/api-keys) tutorial.
