...
A general-purpose key-value store used for sessions, auth and your own data. Pass a plain `Map`, a client for Redis, DynamoDB and the rest, or a [`polystore`](https://polystore.dev/) instance built with the exported `kv`. Off by default, though several features (like [`auth`](#optionsauth)) need it. See [Stores](#stores).
...
server({ store: kv(createClient({ url }).connect()) });
```
