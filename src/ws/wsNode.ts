...
// A frame's length comes straight off the wire (up to 2^64), so cap what we
// are willing to buffer: without one, a single crafted header makes the
// process accumulate memory until it dies. Applies to a frame and to a
// fragmented message as a whole; 1009 is "message too big" (RFC 6455).
const MAX_MESSAGE = 16 * 1024 ** 2;
...
export class NodeWebSocket {
  ...
  receive(chunk: Buffer): void {
    ...
    while (true) {
      ...
      // Refuse before buffering, so the announced size alone cannot hurt us
      if (len > MAX_MESSAGE) {
  ...
  private frame(fin: boolean, opcode: number, payload: Buffer): void {
    ...
    // Each fragment fits the cap on its own; the reassembled message must too
