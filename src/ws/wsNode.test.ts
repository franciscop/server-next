...
describe("wsNode frame codec", () => {
  ...
  it("refuses a frame announcing an oversized payload with a 1009 close", () => {
    const { sock, ws } = make();
    // A 1gb announced length; only the header arrives, no payload needed
    const header = Buffer.alloc(10);
    header[0] = 0x82; // FIN + binary
    header[1] = 0x80 | 127;
    header.writeBigUInt64BE(BigInt(1024 ** 3), 2);
    ws.receive(header);
    expect(ws.readyState).toBe(3);
    const close = decodeServerFrame(sock.writes[0]);
    expect(close.opcode).toBe(0x8);
    expect(close.payload.readUInt16BE(0)).toBe(1009);
  });

  it("refuses an oversized fragmented message with a 1009 close", () => {
    const { sock, ws, messages } = make();
    const chunk = Buffer.alloc(1024 ** 2, 0x61); // 1mb per fragment
    ws.receive(clientFrame(chunk, 0x2, false));
    for (let i = 0; i < 16; i++) ws.receive(clientFrame(chunk, 0x0, false));
    expect(ws.readyState).toBe(3);
