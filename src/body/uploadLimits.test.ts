...
function multipart(
  files: { field: string; name: string; type: string; body: Buffer | string }[],
) {
...
const app = (uploads: any, extra: any = {}) =>
  server({ uploads, ...extra })
    .post("/", (ctx) => ({ body: ctx.body }))
    .test();
...
describe("upload size limits", () => {
  ...
  it("rejects one file over maxFileSize, leaving nothing behind", async () => {
    ...
    const res = await app({ bucket, maxFileSize: "1kb" }).post("/", raw, {
      headers,
    });
  ...
  it("caps a raw body by maxFileSize too", async () => {
    const bucket = realBucket();
    const res = await app({ bucket, maxFileSize: "1kb" }).post(
      "/",
      "x".repeat(5000),
      {
...
describe("maxFiles", () => {
  ...
  it("allows a request at the limit", async () => {
    ...
    const res = await app({ bucket: realBucket(), maxFiles: 3 }).post(
      "/",
      raw,
      {
        headers,
