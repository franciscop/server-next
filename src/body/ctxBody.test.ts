...
describe("ctx.body parse: multipart files", () => {
  ...
  // unconfigured `uploads` is an error rather than a silent no-op.
  it("refuses a file part when no bucket is configured", async () => {
    const body = multipart([
...
describe("ctx.body parse: raw single file (Case B)", () => {
  ...
  // nowhere to put it, is a configuration mistake rather than a Buffer.
  it("refuses a raw file body when no bucket is configured", async () => {
    const api = server()
      .post("/", (ctx) => ctx.body)
      .test();
  ...
  it("still hands back the bytes when files are off on purpose", async () => {
    const api = server({ uploads: false })
      .post("/", (ctx) => ({
