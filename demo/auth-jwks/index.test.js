...
globalThis.fetch = async (url, opts) => {
  if (String(url).startsWith(ISSUER)) return issuer.get(new URL(url).pathname);
...
const token = ({ aud = AUDIENCE, iss = ISSUER, key = privateKey, ...claims }) =>
  new SignJWT(claims)
...
describe("verifying hosted-auth JWTs", () => {
  ...
  it("rejects a token signed by someone else", async () => {
    const { privateKey: mallory } = await generateKeyPair("RS256");
  ...
  it("rejects a token from another issuer", async () => {
    const jwt = await token({ sub: "u1", email: "bad@x.com", iss: "https://evil.example" });
    expect((await meWith(jwt)).status).toBe(401);
  });
